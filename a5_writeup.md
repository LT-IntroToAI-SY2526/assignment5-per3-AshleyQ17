...
The Stack class work by being last in first out and the queue class works by being first in first out. First in first out data structures work by the first one being pushed gets popped before the others since it pops them in sequence from when they got pushed. Last in First Out works by popping first the things that were most recently added, you push an item onto the top. They're both important for DFS and BFS because the programs need to keep track of what to explore next which can be determined by using either. Also usinng a stack versus a queue changes the way the search algorithm explores possible solutions because stack searches in depth one path first before checking the other possible ones, while queue explores all paths before advancing to the next layer. Stack is better for DFS because it ecplores the newest path first, while queue is better for BFS because it explores the newest/closest option first.
